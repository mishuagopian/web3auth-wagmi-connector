...
class Web3AuthConnector extends core_namespaceObject.Connector {
  ...
  async subscribeToAdpaterConnectionEvents() {
    return new Promise((resolve, reject) => {
      this.web3AuthInstance.once(base_namespaceObject.ADAPTER_EVENTS.CONNECTED, async () => {
        ...
        return resolve({
          account: (0,utils_namespaceObject.getAddress)(account),
          chain: {
            id: 0,
            unsupported: false
          },
  ...
  async init() {
    ...
    try {
      ...
      const hasExternalWallets = allAdapters.some(adapterName => {
        return this.adapters[adapterName]?.type === base_namespaceObject.ADAPTER_CATEGORY.EXTERNAL && this.modalConfig.adapters?.[adapterName].showOnModal;
      });
  ...
  async getAccount() {
    ...
    const account = await signer.getAddress();
    return (0,utils_namespaceObject.getAddress)(account);
  }
  async getProvider() {
    if (this.provider) {
  ...
  async initExternalWalletAdapters(externalWalletsInitialized, options) {
    ...
    const adapterPromises = Object.keys(this.adapters).map(async adapterName => {
      try {
        const adapter = this.adapters[adapterName];
        if (adapter?.type === base_namespaceObject.ADAPTER_CATEGORY.EXTERNAL) {
